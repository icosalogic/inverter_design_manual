...
Also note that all the efficiency and power loss graphs shown in this document were generated by the design tool.
...
This graph shows that it may be a good strategy to use a smaller MOSFET when the inverter is
running at light load, then switch to a bigger MOSFET when the load increases.
...
The following graph shows the power loss curve for such a configuration in a 6 kW single phase inverter.
